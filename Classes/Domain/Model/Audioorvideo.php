 ...
 * Audioorvideo
 */
class Audioorvideo extends \TYPO3\CMS\Extbase\DomainObject\AbstractEntity
{
    ...
    */
    ...
    public function getPicture() {
        $orgFileName = $fileName = $_SERVER['DOCUMENT_ROOT'].'/'.$this->picture;
        
        if(!file_exists($fileName)) {
            $this->picture = substr($fileName, 0, strrpos($fileName,'.')).'.png';
            
            if(!file_exists($this->picture)) {
                $this->picture = substr($fileName, 0, strrpos($fileName,'.')).'.svg';
            }
            
            if(!file_exists($this->picture)) {
                $orgFileName = str_replace('/logos/','/logos_old/',$orgFileName);
                
                $this->picture = $orgFileName;
            }
        }
